...
$$x_n = \frac{1}{T}\int_{-\frac{T}{2}}^{\frac{T}{2}}x(t)e^{-j\omega{nt}}dt$$
...
>💡**频域周期延拓和前面的系数$\frac{1}{T_s}$的来源**：冲激串为周期信号->展开成傅里叶级数形式->求得频谱$c_n$恰好为$\frac{1}{T_s}$->用傅里叶级数表达式作FT
...
- 时移
$$ZT[x(n-m)]=z^{-m}X(Z)$$
- 尺度变换:**时域上乘上一个指数函数，频域上拉伸**
$$ZT[a^nx(n)]=X(\frac{Z}{a})$$

>💡尺度变换后ROC也变为原来的|a|倍!

- 微分(**与FT的形式相同**)：
$$ZT[nx(n)]=-z\frac{dX(Z)}{dz}$$
- 卷积(**联想到通信原理中的f**)
$$ZT[x(n)*y(n)]=X(Z)Y(Z)$$
...
1. 对于IIR系统：
   系统函数可以表示为：
$$y[n]=\sum_{i=0}^{N}b_iz^{-i}-\sum_{j=1}^{M}z^{-j}$$
2. 对于FIR系统（**极点在原点，一定稳定，一定因果，易实现线性相位**）：
   系统函数可以表示为：
...
- 分离因式法
- ⭐留数法
>💡联想到复变函数中为了解决一个闭合曲线积分内部有“坑”（极点）的情况提出了**留数定理**，即**一个闭合曲线的积分是曲线内部所有极点的留数之和**

即
$$IZT[X(z)]=Res[X(z)\color{yellow}z^{n-1},z_i]$$
⭐还存在一种广义形式（互补思想）：**逆变换等于曲线外极点留数之和的相反数**：
$$IZT[X(z)]=-Res[X(z)\color{yellow}z^{n-1},z_j]$$
>**一定不要漏了后面的$z^{n-1}$！！！**

当$z_i$为一阶极点时:
$$Res[X(z)z^{n-1},z_i]=[(z-z_i)X(z)z^{n-1}]_{z=z_i}$$
当$z_i$为n阶极点时：
$$Res[X(z)z^{n-1},z_j]=\frac{1}{(n-1)!}\frac{d^{n-1}}{dz^{n-1}}[(z-z_i)^nX(z)z^{n-1}]_{z=z_i}$$
